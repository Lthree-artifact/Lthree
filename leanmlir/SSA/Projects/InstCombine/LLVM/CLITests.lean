  ...
  | ⟨ty::tys⟩, ⟨val::vals,hlen⟩ =>
    ...
    match ty with
      | bitvec w =>
        let newTy : ⟦bitvec w⟧ :=
          match val with
          | some z => .value (BitVec.ofInt w z)
          | none => .poison
        Ctxt.Valuation.cons newTy valuation'
      | .ptr =>
        let newTy : ⟦LLVM.Ty.ptr⟧ :=
          match val with
          | some z => .some (.value { blockId := Int.toNat z, offset := 0#64 })
          | none => .some .poison
        Ctxt.Valuation.cons newTy valuation'
...
instance {test : ConcreteCliTest} : ToString (toType test.ty) where
  toString :=
    ...
    | bitvec _ => fun v =>
      ...
      | .none => "immediateUB"
      | .some .poison => "poison"
      | .some (.value bv) => toString bv
    | .ptr => fun v =>
      match v with
      | .none => "immediateUB"
      | .some .poison => "poison"
      | .some (.value p) => s!"({p.blockId}, {p.offset})"
