...
-/
def EffectM := StateT GlobalState ImmediateUBOr
...
-/
def MemorySSAM := StateT MemoryState ImmediateUBOr
...
instance : HasUB EffectM where
  throwUB := StateT.lift ImmediateUBOr.immediateUB
...
@[simp, simp_denote, simp_sllvm]
lemma run_ub : StateT.run (throwUB : EffectM α) s = .immediateUB := rfl
...
lemma run_map (x : EffectM α) :
    StateT.run (f <$> x : EffectM β) s = (fun p => (f p.1, p.2)) <$> StateT.run x s := by
  cases h : StateT.run x s <;> simp [h]
