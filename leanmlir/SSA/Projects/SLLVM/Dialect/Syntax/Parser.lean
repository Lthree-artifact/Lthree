...
def parseZeroPoisonFlag (op : AST.Op 0) : ReaderM _root_.LLVM.ZeroPoisonFlag :=
  match op.getAttr? "is_zero_poison" with
  | .none => return (_root_.LLVM.ZeroPoisonFlag.mk false)
  | .some _ => return (_root_.LLVM.ZeroPoisonFlag.mk (← op.getBoolAttr "is_zero_poison"))
...
instance : DialectParse SLLVM 0 where
  ...
  match opStx.name with
    ...
    | "llvm.ctpop"  => mkExprOf <| LeanMLIR.SLLVM.Op.arith <| InstCombine.LLVM.Op.ctpop (← unW)
    | "llvm.ctlz" | "llvm.intr.ctlz" =>
        mkExprOf <| LeanMLIR.SLLVM.Op.arith <| InstCombine.LLVM.Op.ctlz (← unW) (← parseZeroPoisonFlag opStx)
    | "llvm.cttz" | "llvm.intr.cttz" =>
        mkExprOf <| LeanMLIR.SLLVM.Op.arith <| InstCombine.LLVM.Op.cttz (← unW) (← parseZeroPoisonFlag opStx)
