...
def runTest (test : ConcreteCliTest) (arg : String) : IO Bool := do
    ...
    match test.ty, res with
      | bitvec _, .none => IO.println s!"no result (immediate undefined behavior)"
      | bitvec _, .some (.value val) => IO.println s!"result: {val}"
      | bitvec _, .some .poison => IO.println s!"no result (undefined behavior)"
      | .ptr, .none => IO.println s!"no result (immediate undefined behavior)"
      | .ptr, .some (.value val) => IO.println s!"result: {val}"
      | .ptr, .some .poison => IO.println s!"no result (undefined behavior)"
    return true
