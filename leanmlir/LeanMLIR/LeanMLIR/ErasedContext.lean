...
@[simp] theorem map_id (Γ : Ctxt Ty) : Γ.map id = Γ := by
  cases Γ
  simp [map]
...
@[simp] theorem toMap_id_castCtxt {Γ : Ctxt Ty} {t : Ty} (v : Ctxt.Var Γ t) :
    (v.toMap (f := _root_.id)).castCtxt (Ctxt.map_id (Γ := Γ)) = v := by
  apply Subtype.ext
  rfl

@[simp] theorem castCtxt_map_id_symm {Γ : Ctxt Ty} {t : Ty} (v : Ctxt.Var Γ t) :
    v.castCtxt (Ctxt.map_id (Γ := Γ)).symm = v.toMap (f := _root_.id) := by
  apply Subtype.ext
  rfl
...
@[simp] abbrev Hom.mapId {Γ : Ctxt Ty} : Γ.Hom (Γ.map _root_.id) :=
  fun _ v => v.toMap (f := _root_.id)

@[simp] abbrev Hom.unmapId {Γ : Ctxt Ty} : (Γ.map _root_.id).Hom Γ :=
  fun _ v => v.castCtxt (Ctxt.map_id (Γ := Γ))

@[simp] theorem Hom.unmapId_toMap {Γ : Ctxt Ty} (v : Γ.Var t) :
    (Hom.unmapId (Γ := Γ)) (v.toMap (f := _root_.id)) = v := by
  simp [Hom.unmapId]

@[simp] theorem Hom.mapId_unmapId {Γ : Ctxt Ty} (v : (Γ.map _root_.id).Var t) :
    (Hom.mapId (Γ := Γ)) ((Hom.unmapId (Γ := Γ)) v) = v := by
  apply Subtype.ext
  rfl
...
@[simp] theorem Valuation.comap_unmapId_toMap {Γ : Ctxt Ty}
    (V : Valuation Γ) (v : Γ.Var t) :
    (V.comap (Hom.unmapId (Γ := Γ))) (v.toMap (f := _root_.id)) = V v := by
  simp
