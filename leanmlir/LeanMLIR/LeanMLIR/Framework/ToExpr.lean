...
def mapCFGTarget {Γ : Ctxt d.Ty} (dE : Q(Dialect)) (_sig : Q(DialectSignature $dE))
    (target : CFGTarget d Γ) :
    Lean.Expr :=
  let ΓE : Q(Ctxt ($d').Ty) := f.mapCtxt Γ
  let nameE := toExpr target.name
  let argTysE := f.mapList target.argTys
  let argsE := f.mapVarVec target.args
  mkAppN (mkConst ``CFGTarget.mk) #[dE, ΓE, nameE, argTysE, argsE]
...
partial def Com.toExprAux {Γ : Ctxt d.Ty} {eff : EffectKind} {ty}
  ...
  | .cfg entry blocks fallback =>
    let entry := toExpr entry
    let blocks := blocks.toExprAux
    let fallback := fallback.toExprAux
    mkAppN (mkConst ``Com.cfg) #[dE, sig, ΓE, tyE, entry, blocks, fallback]
...
partial def CFGTerm.toExprAux {Γ : Ctxt d.Ty} {retTys}
    (term : CFGTerm d Γ retTys) : Lean.Expr :=
  let ΓE : Q(Ctxt ($dE).Ty) := f.mapCtxt Γ
  let retTysE := f.mapList retTys
  match term with
  | .ret vs =>
      let vs := f.mapVarVec vs
      mkAppN (mkConst ``CFGTerm.ret) #[dE, ΓE, retTysE, vs]
  | .br target =>
      let target := f.mapCFGTarget dE sig target
      mkAppN (mkConst ``CFGTerm.br) #[dE, ΓE, retTysE, target]
  | .condBr (condTy := condTy) cond trueTarget falseTarget =>
      let condTyE := f.mapTy condTy
      let cond := f.mapVar cond
      let trueTarget := f.mapCFGTarget dE sig trueTarget
      let falseTarget := f.mapCFGTarget dE sig falseTarget
      mkAppN (mkConst ``CFGTerm.condBr) #[
        dE, ΓE, retTysE, condTyE, cond, trueTarget, falseTarget
      ]

partial def CFGBody.toExprAux {Γ : Ctxt d.Ty} {retTys}
    (body : CFGBody d Γ .impure retTys) : Lean.Expr :=
  let ΓE : Q(Ctxt ($dE).Ty) := f.mapCtxt Γ
  let retTysE := f.mapList retTys
  match body with
  | .terminator term =>
      let term := term.toExprAux
      mkAppN (mkConst ``CFGBody.terminator) #[dE, sig, ΓE, retTysE, term]
  | .var (ty := eTy) e body =>
      let eTyE := f.mapList eTy
      let e := e.toExprAux
      let body := body.toExprAux
      mkAppN (mkConst ``CFGBody.var) #[dE, sig, ΓE, eTyE, retTysE, e, body]

partial def CFGBlock.toExprAux {Γ : Ctxt d.Ty} {retTys}
    (block : CFGBlock d Γ .impure retTys) : Lean.Expr :=
  let ΓE : Q(Ctxt ($dE).Ty) := f.mapCtxt Γ
  let retTysE := f.mapList retTys
  match block with
  | .mk name args body =>
      let name := toExpr name
      let argsE := f.mapCtxt args
      let body := body.toExprAux
      mkAppN (mkConst ``CFGBlock.mk) #[dE, sig, ΓE, retTysE, name, argsE, body]

partial def CFGBlocks.toExprAux {Γ : Ctxt d.Ty} {retTys}
    (blocks : CFGBlocks d Γ .impure retTys) : Lean.Expr :=
  let ΓE : Q(Ctxt ($dE).Ty) := f.mapCtxt Γ
  let retTysE := f.mapList retTys
  match blocks with
  | .nil =>
      mkAppN (mkConst ``CFGBlocks.nil) #[dE, sig, ΓE, retTysE]
  | .cons block blocks =>
      let block := block.toExprAux
      let blocks := blocks.toExprAux
      mkAppN (mkConst ``CFGBlocks.cons) #[dE, sig, ΓE, retTysE, block, blocks]
