...
/-- A CFG branch target, together with the SSA values passed as block arguments. -/
structure CFGTarget (Γ : Ctxt d.Ty) where
  name : String
  argTys : List d.Ty
  args : HVector Γ.Var argTys

/-- CFG terminators live at the `Com` level, not as ordinary dialect operations. -/
inductive CFGTerm : (Γ : Ctxt d.Ty) → (retTys : List d.Ty) → Type where
  | ret {Γ retTys} (vs : HVector Γ.Var retTys) : CFGTerm Γ retTys
  | br {Γ retTys} (target : CFGTarget d Γ) : CFGTerm Γ retTys
  | condBr {Γ retTys condTy}
      (cond : Γ.Var condTy)
      (trueTarget falseTarget : CFGTarget d Γ) : CFGTerm Γ retTys
...
inductive Com : Ctxt d.Ty → EffectKind → List d.Ty → Type where
  ...
  | cfg {Γ} {tys}
      (entry : String)
      (blocks : CFGBlocks Γ .impure tys)
      (fallback : Com Γ .impure tys) : Com Γ .impure tys

/-- The straight-line body of a CFG basic block, ending in a CFG terminator. -/
inductive CFGBody : Ctxt d.Ty → EffectKind → List d.Ty → Type where
  | terminator {Γ retTys} (term : CFGTerm d Γ retTys) : CFGBody Γ .impure retTys
  | var (e : Expr Γ .impure ty) (body : CFGBody (ty ++ Γ) .impure retTys) :
      CFGBody Γ .impure retTys

/-- A CFG basic block. Its body is typed in the block-argument context followed by
the function-entry context. -/
inductive CFGBlock : Ctxt d.Ty → EffectKind → List d.Ty → Type where
  | mk {Γ retTys}
      (name : String)
      (args : Ctxt d.Ty)
      (body : CFGBody (args ++ Γ) .impure retTys) :
      CFGBlock Γ .impure retTys

/-- A list of CFG blocks indexed by their shared entry context and return types. -/
inductive CFGBlocks : Ctxt d.Ty → EffectKind → List d.Ty → Type where
  | nil {Γ retTys} : CFGBlocks Γ .impure retTys
  | cons {Γ retTys} (block : CFGBlock Γ .impure retTys)
      (blocks : CFGBlocks Γ .impure retTys) : CFGBlocks Γ .impure retTys
end
...
noncomputable instance HVector.decidableEqReg [DecidableEq d.Op] [DecidableEq d.Ty] :
    ∀ {l : RegionSignature d.Ty}, DecidableEq (HVector (fun t => Com d t.1 .impure t.2) l)
  | _ => Classical.decEq _

noncomputable instance CFGTarget.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} : DecidableEq (CFGTarget d Γ) :=
  Classical.decEq _

noncomputable instance CFGTerm.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} {tys : List d.Ty} : DecidableEq (CFGTerm d Γ tys) :=
  Classical.decEq _

noncomputable instance Expr.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty] :
    {Γ : Ctxt d.Ty} → {ty : List d.Ty} → DecidableEq (Expr d Γ eff ty)
  | _, _ => Classical.decEq _

noncomputable instance Com.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} {eff : EffectKind} {tys : List d.Ty} : DecidableEq (Com d Γ eff tys)
  := Classical.decEq _

noncomputable instance CFGBody.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} {eff : EffectKind} {tys : List d.Ty} : DecidableEq (CFGBody d Γ eff tys)
  := Classical.decEq _

noncomputable instance CFGBlock.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} {eff : EffectKind} {tys : List d.Ty} : DecidableEq (CFGBlock d Γ eff tys)
  := Classical.decEq _

noncomputable instance CFGBlocks.decidableEq [DecidableEq d.Op] [DecidableEq d.Ty]
    {Γ : Ctxt d.Ty} {eff : EffectKind} {tys : List d.Ty} : DecidableEq (CFGBlocks d Γ eff tys)
  := Classical.decEq _
...
section Rec
variable {t} {motive : ∀ {Γ eff}, Com d Γ eff t → Sort u}
          (retsCase : ∀ {Γ : Ctxt _} {eff : EffectKind},
            (v : HVector Γ.Var t) → motive (Com.rets (eff := eff) v))
          (varCase : ∀ {Γ eff} {u},
            (e : Expr d Γ eff u) → (body : Com d e.outContext eff t) →
              motive body → motive (Com.var e body))
          (cfgCase : ∀ {Γ : Ctxt _} (entry : String)
            (blocks : CFGBlocks d Γ .impure t) (fallback : Com d Γ .impure t),
              motive fallback → motive (Com.cfg entry blocks fallback))

@[elab_as_elim]
def Com.recAux' {Γ eff} : (com : Com d Γ eff t) → motive com
  | Com.rets v => retsCase v
  | Com.var e body => varCase e body (Com.recAux' body)
  | Com.cfg entry blocks fallback => cfgCase entry blocks fallback (Com.recAux' fallback)
...
def Com.rec' {Γ eff} (com : Com d Γ eff t) : motive com :=
  Com.recAux' retsCase varCase cfgCase com

variable {retsCase} {varCase} {cfgCase} {Γ : Ctxt _} {eff : EffectKind}

@[simp] theorem Com.rec'_rets (v : HVector Γ.Var t) :
    (Com.rets (d:=d) (eff := eff) v).rec' (motive:=motive) retsCase varCase cfgCase
      = retsCase v :=
  rfl

@[simp] theorem Com.rec'_var (e : Expr d Γ eff u) (body : Com d _ _ t) :
    (Com.var e body).rec' (motive:=motive) retsCase varCase cfgCase
    = varCase e body (body.rec' (motive:=motive) retsCase varCase cfgCase) :=
  rfl

@[simp] theorem Com.rec'_cfg (entry : String) (blocks : CFGBlocks d Γ .impure t)
    (fallback : Com d Γ .impure t) :
    (Com.cfg entry blocks fallback).rec' (motive:=motive) retsCase varCase cfgCase
    = cfgCase entry blocks fallback
        (fallback.rec' (motive:=motive) retsCase varCase cfgCase) :=
  rfl

omit retsCase varCase cfgCase in
theorem Com.recAux'_eq :
    Com.recAux' (motive:=motive) = Com.rec' (motive:=motive) := by
  funext retsCase varCase cfgCase Γ eff com
  induction com using Com.rec' <;> simp [recAux', *]

end Rec

section RecPure
variable {t} {motive : ∀ {Γ}, Com d Γ .pure t → Sort u}
          (retsCase : ∀ {Γ : Ctxt _},
            (v : HVector Γ.Var t) → motive (Com.rets (eff := .pure) v))
          (varCase : ∀ {Γ} {u},
            (e : Expr d Γ .pure u) → (body : Com d e.outContext .pure t) →
              motive body → motive (Com.var e body))

@[elab_as_elim, induction_eliminator]
def Com.recPure {Γ} : (com : Com d Γ .pure t) → motive com
  | Com.rets v => retsCase v
  | Com.var e body => varCase e body (Com.recPure body)
...
def Com.size : Com d Γ eff t → Nat
  | .rets _ => 0
  | .var _ body => body.size + 1
  | .cfg _ _ fallback => fallback.size
...
def Com.bvars : Com d Γ eff t → Nat
  | .rets _ => 0
  | .var e body => e.bvars + body.bvars
  | .cfg _ _ fallback => fallback.bvars
...
def Com.outContext {Γ} : Com d Γ eff ts → Ctxt d.Ty :=
  fun
  | .rets _ => Γ
  | .var _ body => body.outContext
  | .cfg _ _ fallback => fallback.outContext
...
def Com.outContextDiff : (com : Com d Γ eff ts) → Γ.Diff com.outContext
  | .rets _ =>
      ⟨0, by intro _ _ h; exact h⟩
  | .var e body =>
      ⟨e.bvars + body.outContextDiff.val, by
        intro i t h
        simpa [Com.outContext, Nat.add_assoc] using
          body.outContextDiff.property (i := i + e.bvars) (t := t) (by
            rw [Ctxt.get_add_bvars e, h])⟩
  | .cfg _ _ fallback =>
      fallback.outContextDiff
...
def Com.returnVars : (com : Com d Γ eff ts) → HVector (Var com.outContext) ts
  ...
  | .cfg _ _ fallback => fallback.returnVars
...
@[simp] theorem Com.outContext_var {eff} (e : Expr d Γ eff t) (body : Com d (t ++ Γ) eff u) :
    (Com.var e body).outContext = body.outContext := by
  rfl
...
@[simp] theorem Com.outContextHom_var :
    ...
    = (e.contextHom.comp body.outContextHom).castCodomain (Com.outContext_var e body).symm := by
  funext t v
  apply Subtype.ext
...
@[simp] theorem Com.returnVars_var {Γ : Ctxt d.Ty} {eff : EffectKind} {ty retTy : List d.Ty}
    (e : Expr d Γ eff ty) (body : Com d (ty ++ Γ) eff retTy) :
    returnVars (Com.var e body) = body.returnVars := by
  rfl
...
def Com.denote : Com d Γ eff ty → (Γv : Valuation Γ) →
  ...
  | .cfg _ _ fallback, V => fallback.denote V
end
...
def Com.denoteLets : (com : Com d Γ eff ty) → (Γv : Valuation Γ) →
  ...
  | .cfg _ _ fallback, V => fallback.denoteLets V
...
def CFGTarget.changeVars (varsMap : Γ.Hom Γ') (target : CFGTarget d Γ) :
    CFGTarget d Γ' where
  name := target.name
  argTys := target.argTys
  args := target.args.map varsMap

def CFGTerm.changeVars (varsMap : Γ.Hom Γ') :
    CFGTerm d Γ retTys → CFGTerm d Γ' retTys
  | .ret vs => .ret (vs.map varsMap)
  | .br target => .br (target.changeVars varsMap)
  | .condBr cond trueTarget falseTarget =>
      .condBr (varsMap cond) (trueTarget.changeVars varsMap) (falseTarget.changeVars varsMap)

def CFGBody.changeVars : CFGBody d Γ .impure retTys →
    (varsMap : Γ.Hom Γ') → CFGBody d Γ' .impure retTys
  | .terminator term, varsMap => .terminator (term.changeVars varsMap)
  | .var e body, varsMap => .var (e.changeVars varsMap)
      (body.changeVars (fun _ v => varsMap.append v))

def CFGBlock.changeVars (varsMap : Γ.Hom Γ') :
    CFGBlock d Γ .impure retTys → CFGBlock d Γ' .impure retTys
  | .mk name args body => .mk name args (body.changeVars (varsMap.append (ζ := args)))

def CFGBlocks.changeVars (varsMap : Γ.Hom Γ') :
    CFGBlocks d Γ .impure retTys → CFGBlocks d Γ' .impure retTys
  | .nil => .nil
  | .cons block blocks => .cons (block.changeVars varsMap) (blocks.changeVars varsMap)
...
def Com.changeVars : Com d Γ eff ty →
  ...
  |  .var e body => fun varsMap => .var (e.changeVars varsMap)
      (body.changeVars (fun _ v => varsMap.append v))
  |  .cfg entry blocks fallback => fun varsMap =>
      .cfg entry (blocks.changeVars varsMap) (fallback.changeVars varsMap)
...
@[simp] theorem Com.denote_changeVars
  ...
  | retsCase x      => simp [HVector.map_map]; rfl
  | varCase _ _ ih => simp [denote, ih]
  | cfgCase _ _ _ ih => simpa [Com.changeVars, denote] using ih varsMap
...
@[simp] def Com.outContext_changeVars_hom {map : Γ.Hom Δ} (map_inv : Δ.Hom Γ) :
  ...
  | .cfg _ _ fallback => Com.outContext_changeVars_hom (map := map) map_inv (c := fallback)
...
@[simp] theorem Com.denoteLets_returnVars (c : Com d Γ .pure tys) (V : Valuation Γ) :
  ...
  case retsCase v  => rfl
  case varCase e body ih =>
    simp [denoteLets, Id.pure_eq', Id.bind_eq', denote]
    exact ih (e.denote V)
...
@[simp] theorem CFGTarget.changeVars_id (target : CFGTarget d Γ) :
    target.changeVars .id = target := by
  cases target
  simp [CFGTarget.changeVars]

@[simp] theorem CFGTerm.changeVars_id (term : CFGTerm d Γ t) :
    term.changeVars .id = term := by
  cases term <;> simp [CFGTerm.changeVars]

@[simp] theorem CFGBody.changeVars_id : (body : CFGBody d Γ .impure t) →
    body.changeVars .id = body
  | .terminator term => by simp [CFGBody.changeVars]
  | .var e body => by simp [CFGBody.changeVars, CFGBody.changeVars_id body]

@[simp] theorem CFGBlock.changeVars_id (block : CFGBlock d Γ .impure t) :
    block.changeVars .id = block := by
  cases block with
  | mk name args body =>
      simp [CFGBlock.changeVars]

@[simp] theorem CFGBlocks.changeVars_id : (blocks : CFGBlocks d Γ .impure t) →
    blocks.changeVars .id = blocks
  | .nil => by simp [CFGBlocks.changeVars]
  | .cons block blocks => by simp [CFGBlocks.changeVars, CFGBlocks.changeVars_id blocks]
...
def Com.changeEffect {eff₁ eff₂ : EffectKind} (h : eff₁ ≤ eff₂) :
  ...
  | .rets v => .rets v
  | .var e body => .var (e.changeEffect h) (body.changeEffect h)
  | .cfg entry blocks fallback => by
      cases eff₂
      · exact False.elim (EffectKind.not_impure_le_pure h)
      · exact .cfg entry blocks fallback
...
@[simp] theorem Com.returnVars_castPureToEff (eff : _) (com : Com d Γ .pure tys) :
  ...
  induction com using Com.recPure with
  | retsCase => simp
  | varCase e body ih =>
      simpa [Com.returnVars_var] using ih
...
def CFGTarget.changeDialect (target : CFGTarget d Γ) :
    CFGTarget d' (Γ.map f.mapTy) where
  name := target.name
  argTys := target.argTys.map f.mapTy
  args := target.args.map' f.mapTy (fun _ v => v.toMap)

def CFGTerm.changeDialect : CFGTerm d Γ retTys → CFGTerm d' (Γ.map f.mapTy) (retTys.map f.mapTy)
  | .ret vs => .ret (vs.map' f.mapTy (fun _ v => v.toMap))
  | .br target => .br (target.changeDialect f)
  | .condBr cond trueTarget falseTarget =>
      .condBr cond.toMap (trueTarget.changeDialect f) (falseTarget.changeDialect f)
...
def Com.changeDialect : Com d Γ eff ty → Com d' (f.mapTy <$> Γ) eff (f.mapTy <$> ty)
  ...
  | .cfg entry blocks fallback =>
      .cfg entry blocks.changeDialect fallback.changeDialect
...
def CFGBody.changeDialect : CFGBody d Γ eff retTys →
    CFGBody d' (Γ.map f.mapTy) eff (retTys.map f.mapTy)
  | .terminator term => .terminator (term.changeDialect f)
  | .var e body =>
      let body :=
        body.changeDialect.changeVars <|
          Hom.id.castCodomain (by simp; rfl)
      .var e.changeDialect body
termination_by body => sizeOf body

def CFGBlock.changeDialect : CFGBlock d Γ eff retTys →
    CFGBlock d' (Γ.map f.mapTy) eff (retTys.map f.mapTy)
  | .mk name args body =>
      .mk name (args.map f.mapTy) <|
        cast (by simp [Ctxt.map_append]) body.changeDialect
termination_by block => sizeOf block

def CFGBlocks.changeDialect : CFGBlocks d Γ eff retTys →
    CFGBlocks d' (Γ.map f.mapTy) eff (retTys.map f.mapTy)
  | .nil => .nil
  | .cons block blocks => .cons block.changeDialect blocks.changeDialect
termination_by blocks => sizeOf blocks
...
@[simp] theorem Expr.changeDialect_op (f : DialectMorphism d d') (e : Expr d Γ eff t) :
    (e.changeDialect f).op = f.mapOp e.op := by
  cases e with
  | mk op ty_eq eff_le args regArgs =>
      cases ty_eq
      simp [Expr.changeDialect]

@[simp] theorem Expr.changeDialect_sig (f : DialectMorphism d d') (e : Expr d Γ eff t) :
    DialectSignature.sig (e.changeDialect f).op = DialectSignature.sig (f.mapOp e.op) := by
  simp [Expr.changeDialect_op]

@[simp] theorem Expr.changeDialect_regSig (f : DialectMorphism d d') (e : Expr d Γ eff t) :
    DialectSignature.regSig (e.changeDialect f).op = DialectSignature.regSig (f.mapOp e.op) := by
  simp [Expr.changeDialect_op]

@[simp] theorem Expr.changeDialect_returnTypes (f : DialectMorphism d d') (e : Expr d Γ eff t) :
    DialectSignature.returnTypes (e.changeDialect f).op = DialectSignature.returnTypes (f.mapOp e.op) := by
  simp [Expr.changeDialect_op]

@[simp] theorem Expr.changeDialect_effectKind (f : DialectMorphism d d') (e : Expr d Γ eff t) :
    DialectSignature.effectKind (e.changeDialect f).op = DialectSignature.effectKind (f.mapOp e.op) := by
  simp [Expr.changeDialect_op]
...
def Lets.addComToEnd {Γ_out} {eff} (lets : Lets d Γ_in eff Γ_out) :
  ...
  | Com.rets _       => lets
  | Com.var e body => cast (by cases e; rfl) (addComToEnd (Lets.var lets e) body)
  | Com.cfg _ _ fallback => addComToEnd lets fallback
...
@[simp] theorem Lets.addComToEnd_var {lets : Lets d Γ_in eff Γ_out}
    {e : Expr d Γ_out eff tys} {com : Com d (tys ++ Γ_out) eff t} :
    addComToEnd lets (Com.var e com) = addComToEnd (lets.var e) com := by
  simp [addComToEnd]
...
@[simp] theorem Lets.addComToEnd_castPureToEff {lets : Lets d Γ_in .pure Γ_out}
  ...
  case retsCase => simp
  case varCase ih =>
    simp only [Com.castPureToEff_var, Lets.addComToEnd_var,
...
@[simp] theorem Lets.denote_addComToEnd
  ...
  case retsCase => simp [Com.denoteLets]
  case varCase ih => simp [addComToEnd, ih, denote_var]
  case cfgCase ih =>
    funext V
    simp [addComToEnd, Com.denoteLets, ih]
