...
partial def CFGTarget.print (target : CFGTarget d Γ) : Format :=
  let vals := target.args.mapToList printVar
  let tys := target.argTys.map printTy
  let args :=
    (vals.zip tys).map (fun (v, t) => f!"{v} : {t}")
    |> (Format.joinSep · ", ")
    |> Format.group
  f!"^{target.name}({args})"

partial def CFGTerm.print : CFGTerm d Γ ts → Format
  | .ret vs =>
      let vs :=
  ...
  | .br target =>
      Format.align true ++ f!"\"llvm.br\" {target.print}"
  | .condBr cond trueTarget falseTarget =>
      Format.align true ++ f!"\"llvm.cond_br\" {printVar cond}, {trueTarget.print}, {falseTarget.print}"

partial def CFGBody.printAux : CFGBody d Γ .impure ts → Format
  | .terminator term => term.print
  | .var e body => e.print ++ CFGBody.printAux body

partial def CFGBlock.print : CFGBlock d Γ .impure ts → Format
  | .mk name args body =>
      Format.align true ++ f!"^{name}{printBlockArgs args}:\n"
      ++ Format.nest 2 body.printAux

partial def CFGBlocks.print : CFGBlocks d Γ .impure ts → Format
  | .nil => f!""
  | .cons block blocks => block.print ++ blocks.print
...
private partial def Com.printAux : Com d Γ eff ts → Format
  ...
  | .cfg _ blocks _fallback => blocks.print
...
partial def Com.print (com : Com d Γ eff ts) : Format :=
  match com with
  | .cfg _ blocks _fallback =>
      f!"\{\n" ++ Format.nest 2 blocks.print ++ Format.align true ++ f!"}"
  | _ =>
      f!"\{\n"
      ++ (Format.nest 2 <|
