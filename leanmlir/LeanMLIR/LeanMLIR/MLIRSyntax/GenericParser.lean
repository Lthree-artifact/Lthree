...
/-- Interpret a de-Bruijn-like width index as a metavariable when in range.
If the index is out of range for the current `φ`, fall back to a concrete width. -/
private def widthAtIdx (idx : Nat) : Width φ :=
  if h : idx < φ then
    .mvar ⟨idx, h⟩
  else
    .concrete idx
...
macro_rules
  | `([mlir_type| $x:ident ]) => do
        ...
        else if xstr.front == 'w'
        then do
          let xstr' := (xstr.drop 1).toString
          match xstr'.toNat? with
          | some 0 =>
              Macro.throwErrorAt x "width metavariable names must start at w1 (e.g. w1, w2)"
          | some n =>
              let idx := n - 1
              let idxLit := Lean.Syntax.mkNumLit (toString idx)
              `(MLIRType.int Signedness.Signless (widthAtIdx (φ := _) $idxLit))
          | none =>
              Macro.throwErrorAt x $ "cannot convert suffix of w to nat: " ++ xstr.toString
...
syntax ident ":" mlir_type : mlir_attr_val
...
| `([mlir_attr_val| $x:ident : $t:mlir_type]) =>
    `(AttrValue.int (($x : Int)) [mlir_type| $t])
...
fun {φ} => { name := "entry", args := [], ops := [], blocks := [] }
-/
