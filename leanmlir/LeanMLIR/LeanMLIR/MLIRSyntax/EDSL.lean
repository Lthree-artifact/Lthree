...
def elabIntoCom (region : TSyntax `mlir_region) (d : Q(Dialect)) {φ : Q(Nat)}
    ...
    (_decEqTy          : Q(DecidableEq ($d).Ty) := by exact q(by infer_instance))
    (_decEqOp          : Q(DecidableEq ($d).Op) := by exact q(by infer_instance))
    (_toStringTy       : Q(ToString ($d).Ty) := by exact q(by infer_instance))
