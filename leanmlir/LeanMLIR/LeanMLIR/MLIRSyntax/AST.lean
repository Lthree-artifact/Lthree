...
partial def rgn_to_format {φ} : Region φ → Format
  | Region.mk name args ops blocks =>
      ...
      let doc_blocks :=
        blocks.map (Format.line ++ rgn_to_format ·) |> Format.join
      f!"^{name}{doc_args} : " ++ doc_ops ++ doc_blocks
end
...
def Region.appendOp (bb: Region φ) (op: Op φ): Region φ :=
  match bb with
  | Region.mk name args bbs blocks => Region.mk name args (bbs ++ [op]) blocks

def Region.appendOps (bb: Region φ) (ops: List (Op φ)): Region φ :=
  match bb with
  | Region.mk name args bbs blocks => Region.mk name args (bbs ++ ops) blocks
...
def Region.setArgs {φ} (bb: Region φ) (args: List (SSAVal × MLIRType φ)) : Region φ :=
  match bb with
    | (Region.mk name _ ops blocks) => (Region.mk name args ops blocks)
