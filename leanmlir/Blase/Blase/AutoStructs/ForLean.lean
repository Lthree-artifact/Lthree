...
theorem Std.HashMap.insert_keys_perm_new [BEq K] [LawfulBEq K] [Hashable K] [LawfulHashable K] (m : Std.HashMap K V) (k : K) (v : V) :
  ...
  intro hk
  refine (List.perm_ext_iff_of_nodup (keys_nodup (m := m.insert k v)) ?_).2 ?_
  · exact List.nodup_cons.mpr ⟨(by
      intro hkm
      exact hk ((mem_keys).1 hkm)), keys_nodup (m := m)⟩
  · intro x
    constructor
    · intro h
      have hMap : x ∈ m.insert k v := (mem_keys).1 h
      rcases mem_insert.mp hMap with hka | hm
      · have hkEq : k = x := by simpa [beq_iff_eq] using hka
        exact (List.mem_cons).2 (Or.inl hkEq.symm)
      · exact (List.mem_cons).2 (Or.inr ((mem_keys).2 hm))
    · intro h
      have hMap : x ∈ m.insert k v := by
        rcases (List.mem_cons).1 h with hEq | hm
        · exact mem_insert.mpr (Or.inl (by simpa [beq_iff_eq] using hEq.symm))
        · exact mem_insert.mpr (Or.inr ((mem_keys).1 hm))
      exact (mem_keys).2 hMap
