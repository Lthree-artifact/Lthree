...
local instance licm_exitvalue_mul_nuw_kept_decRefSemVal (a b : LLVM.SemVal (BitVec 21)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance licm_exitvalue_mul_nuw_kept_decRefIntW (a b : LLVM.IntW 21) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 21)) (LLVM.SemVal (BitVec 21)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance licm_exitvalue_mul_nuw_kept_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 21)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 21)) (ImmediateUBOr (LLVM.IntW 21))
        inferInstance a b) := inferInstance
  exact d

local instance licm_exitvalue_mul_nuw_kept_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 21]) : Decidable (a ⊑ b) :=
  match a, b with
...
def licm_exitvalue_mul_nuw_kept_cexInputs : InstCombine.InputValuation
    (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 21]) :=
  licm_exitvalue_mul_nuw_kept_ivOfHVector
    (.cons (licm_exitvalue_mul_nuw_kept_valueOfNat 21 31) .nil)
...
abbrev licm_exitvalue_mul_nuw_kept_tgt_impure :
    Com InstCombine.LLVM (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 21]) .impure
      [InstCombine.LLVM.Ty.bitvec 21] :=
  (licm_exitvalue_mul_nuw_kept_tgt 21).castPureToEff .impure

theorem licm_exitvalue_mul_nuw_kept_src_terminates_at_cexFuel :
    (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel licm_exitvalue_mul_nuw_kept_cexFuel (licm_exitvalue_mul_nuw_kept_src 21)
...
def licm_exitvalue_mul_nuw_kept_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 21]
...
theorem licm_exitvalue_mul_nuw_kept_refute_of_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 21]
...
theorem licm_exitvalue_mul_nuw_kept_correct_counterexample_witness :
    ¬ (InstCombine.IsRefinedByOnIntWInputsWithMemory (licm_exitvalue_mul_nuw_kept_src 21) licm_exitvalue_mul_nuw_kept_tgt_impure) :=
...
theorem licm_exitvalue_mul_nuw_kept_audit_relation_is_memory :
    ((licm_exitvalue_mul_nuw_kept_src 21) ⊑ licm_exitvalue_mul_nuw_kept_tgt_impure)
      = InstCombine.IsRefinedByOnIntWInputsWithMemory (licm_exitvalue_mul_nuw_kept_src 21) licm_exitvalue_mul_nuw_kept_tgt_impure := rfl

theorem licm_exitvalue_mul_nuw_kept_correct_counterexample_bare : ¬ ((licm_exitvalue_mul_nuw_kept_src 21) ⊑ licm_exitvalue_mul_nuw_kept_tgt_impure) :=
...
theorem licm_exitvalue_mul_nuw_kept_correct_counterexample :
    ∃ (w : Nat), ¬ (licm_exitvalue_mul_nuw_kept_src w ⊑ (licm_exitvalue_mul_nuw_kept_tgt w).castPureToEff .impure) :=
  ⟨21, licm_exitvalue_mul_nuw_kept_correct_counterexample_bare⟩

theorem licm_exitvalue_mul_nuw_kept_correct_counterexample_fuel :
    ∃ (V : InstCombine.InputValuation (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 21]))
      (s : InstCombine.LLVMMemory.State) (fuel : Nat) (_hf : 0 < fuel),
      ¬ (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel fuel (licm_exitvalue_mul_nuw_kept_src 21)
