...
local instance licm_exitvalue_sub_add_nuw_kept_decRefSemVal (a b : LLVM.SemVal (BitVec 10)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance licm_exitvalue_sub_add_nuw_kept_decRefIntW (a b : LLVM.IntW 10) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 10)) (LLVM.SemVal (BitVec 10)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance licm_exitvalue_sub_add_nuw_kept_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 10)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 10)) (ImmediateUBOr (LLVM.IntW 10))
        inferInstance a b) := inferInstance
  exact d

local instance licm_exitvalue_sub_add_nuw_kept_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 10]) : Decidable (a ⊑ b) :=
  match a, b with
...
def licm_exitvalue_sub_add_nuw_kept_cexInputs : InstCombine.InputValuation
    (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 10, InstCombine.LLVM.Ty.bitvec 10]) :=
  licm_exitvalue_sub_add_nuw_kept_ivOfHVector
    (.cons (licm_exitvalue_sub_add_nuw_kept_valueOfNat 10 0) (.cons (licm_exitvalue_sub_add_nuw_kept_valueOfNat 10 1) .nil))

def licm_exitvalue_sub_add_nuw_kept_cexState : InstCombine.LLVMMemory.State := default

def licm_exitvalue_sub_add_nuw_kept_cexFuel : Nat := 5

abbrev licm_exitvalue_sub_add_nuw_kept_tgt_impure :
    Com InstCombine.LLVM (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 10, InstCombine.LLVM.Ty.bitvec 10]) .impure
      [InstCombine.LLVM.Ty.bitvec 10] :=
  (licm_exitvalue_sub_add_nuw_kept_tgt 10).castPureToEff .impure

theorem licm_exitvalue_sub_add_nuw_kept_src_terminates_at_cexFuel :
    (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel licm_exitvalue_sub_add_nuw_kept_cexFuel (licm_exitvalue_sub_add_nuw_kept_src 10)
...
def licm_exitvalue_sub_add_nuw_kept_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 10]
...
theorem licm_exitvalue_sub_add_nuw_kept_refute_of_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 10]
...
theorem licm_exitvalue_sub_add_nuw_kept_correct_counterexample_witness :
    ¬ (InstCombine.IsRefinedByOnIntWInputsWithMemory (licm_exitvalue_sub_add_nuw_kept_src 10) licm_exitvalue_sub_add_nuw_kept_tgt_impure) :=
...
theorem licm_exitvalue_sub_add_nuw_kept_audit_relation_is_memory :
    ((licm_exitvalue_sub_add_nuw_kept_src 10) ⊑ licm_exitvalue_sub_add_nuw_kept_tgt_impure)
      = InstCombine.IsRefinedByOnIntWInputsWithMemory (licm_exitvalue_sub_add_nuw_kept_src 10) licm_exitvalue_sub_add_nuw_kept_tgt_impure := rfl

theorem licm_exitvalue_sub_add_nuw_kept_correct_counterexample_bare : ¬ ((licm_exitvalue_sub_add_nuw_kept_src 10) ⊑ licm_exitvalue_sub_add_nuw_kept_tgt_impure) :=
...
theorem licm_exitvalue_sub_add_nuw_kept_correct_counterexample :
    ∃ (w : Nat), ¬ (licm_exitvalue_sub_add_nuw_kept_src w ⊑ (licm_exitvalue_sub_add_nuw_kept_tgt w).castPureToEff .impure) :=
  ⟨10, licm_exitvalue_sub_add_nuw_kept_correct_counterexample_bare⟩

theorem licm_exitvalue_sub_add_nuw_kept_correct_counterexample_fuel :
    ∃ (V : InstCombine.InputValuation (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 10, InstCombine.LLVM.Ty.bitvec 10]))
      (s : InstCombine.LLVMMemory.State) (fuel : Nat) (_hf : 0 < fuel),
      ¬ (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel fuel (licm_exitvalue_sub_add_nuw_kept_src 10)
