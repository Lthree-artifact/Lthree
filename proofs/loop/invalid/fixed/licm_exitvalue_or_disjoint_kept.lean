...
local instance licm_exitvalue_or_disjoint_kept_decRefSemVal (a b : LLVM.SemVal (BitVec 64)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance licm_exitvalue_or_disjoint_kept_decRefIntW (a b : LLVM.IntW 64) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 64)) (LLVM.SemVal (BitVec 64)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance licm_exitvalue_or_disjoint_kept_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 64)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 64)) (ImmediateUBOr (LLVM.IntW 64))
        inferInstance a b) := inferInstance
  exact d

local instance licm_exitvalue_or_disjoint_kept_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 64]) : Decidable (a ⊑ b) :=
  match a, b with
...
def licm_exitvalue_or_disjoint_kept_cexInputs : InstCombine.InputValuation
  ...
  licm_exitvalue_or_disjoint_kept_ivOfHVector
    (.cons (licm_exitvalue_or_disjoint_kept_valueOfNat 64 0) (.cons (licm_exitvalue_or_disjoint_kept_valueOfNat 64 5) .nil))

def licm_exitvalue_or_disjoint_kept_cexState : InstCombine.LLVMMemory.State := default

def licm_exitvalue_or_disjoint_kept_cexFuel : Nat := 8
...
abbrev licm_exitvalue_or_disjoint_kept_tgt_impure :
  ...
  licm_exitvalue_or_disjoint_kept_tgt.castPureToEff .impure

theorem licm_exitvalue_or_disjoint_kept_src_terminates_at_cexFuel :
    (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel licm_exitvalue_or_disjoint_kept_cexFuel licm_exitvalue_or_disjoint_kept_src
...
def licm_exitvalue_or_disjoint_kept_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 64]
...
theorem licm_exitvalue_or_disjoint_kept_refute_of_sepBool (A B : ImmediateUBOr (HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 64]
...
theorem licm_exitvalue_or_disjoint_kept_correct_counterexample_witness :
    ¬ (InstCombine.IsRefinedByOnIntWInputsWithMemory licm_exitvalue_or_disjoint_kept_src licm_exitvalue_or_disjoint_kept_tgt_impure) :=
...
theorem licm_exitvalue_or_disjoint_kept_audit_relation_is_memory :
    (licm_exitvalue_or_disjoint_kept_src ⊑ licm_exitvalue_or_disjoint_kept_tgt_impure)
      = InstCombine.IsRefinedByOnIntWInputsWithMemory licm_exitvalue_or_disjoint_kept_src licm_exitvalue_or_disjoint_kept_tgt_impure := rfl

theorem licm_exitvalue_or_disjoint_kept_correct_counterexample_bare : ¬ (licm_exitvalue_or_disjoint_kept_src ⊑ licm_exitvalue_or_disjoint_kept_tgt_impure) :=
...
theorem licm_exitvalue_or_disjoint_kept_correct_counterexample :
    ∃ (V : InstCombine.InputValuation (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 64, InstCombine.LLVM.Ty.bitvec 64]))
      ...
      ¬ (InstCombine.LLVMMemory.Com.denoteWithMemoryFuel fuel licm_exitvalue_or_disjoint_kept_src
