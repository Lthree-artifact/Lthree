...
local instance pr140526_smin_sdiv_decRefSemVal (a b : LLVM.SemVal (BitVec 2)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance pr140526_smin_sdiv_decRefIntW (a b : LLVM.IntW 2) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 2)) (LLVM.SemVal (BitVec 2)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance pr140526_smin_sdiv_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 2)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 2)) (ImmediateUBOr (LLVM.IntW 2))
        inferInstance a b) := inferInstance
  exact d

local instance pr140526_smin_sdiv_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 2]) : Decidable (a ⊑ b) :=
  match a, b with
...
def pr140526_smin_sdiv_cexInputs : InstCombine.InputValuation
  ...
  pr140526_smin_sdiv_ivOfHVector
    (.cons (pr140526_smin_sdiv_valueOfNat 2 3) (.cons (pr140526_smin_sdiv_valueOfNat 2 1) (.cons (pr140526_smin_sdiv_valueOfNat 2 0) .nil)))

theorem pr140526_smin_sdiv_correct_counterexample_witness : ¬ (pr140526_smin_sdiv_src 2 ⊑ pr140526_smin_sdiv_tgt 2) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (pr140526_smin_sdiv_src 2) (pr140526_smin_sdiv_tgt 2) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (pr140526_smin_sdiv_src 2) (pr140526_smin_sdiv_tgt 2)).mp h
...
theorem pr140526_smin_sdiv_correct_counterexample :
    ∃ (w : Nat), ¬ (pr140526_smin_sdiv_src w ⊑ pr140526_smin_sdiv_tgt w) :=
  ⟨2, pr140526_smin_sdiv_correct_counterexample_witness⟩
