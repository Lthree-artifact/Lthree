...
def pr32949_cexInputs : InstCombine.InputValuation
    (Ctxt.ofList [InstCombine.LLVM.Ty.bitvec 2]) :=
  pr32949_ivOfHVector
    (.cons (pr32949_valueOfNat 2 3) .nil)

theorem pr32949_correct_counterexample_witness : ¬ (pr32949_src 2 ⊑ pr32949_tgt 2) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (pr32949_src 2) (pr32949_tgt 2) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (pr32949_src 2) (pr32949_tgt 2)).mp h
...
theorem pr32949_correct_counterexample :
    ∃ (w : Nat), ¬ (pr32949_src w ⊑ pr32949_tgt w) :=
  ⟨2, pr32949_correct_counterexample_witness⟩
