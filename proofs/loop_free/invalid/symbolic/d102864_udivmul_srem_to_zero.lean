...
local instance d102864_udivmul_srem_to_zero_decRefSemVal (a b : LLVM.SemVal (BitVec 3)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance d102864_udivmul_srem_to_zero_decRefIntW (a b : LLVM.IntW 3) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 3)) (LLVM.SemVal (BitVec 3)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance d102864_udivmul_srem_to_zero_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 3)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 3)) (ImmediateUBOr (LLVM.IntW 3))
        inferInstance a b) := inferInstance
  exact d

local instance d102864_udivmul_srem_to_zero_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 3]) : Decidable (a ⊑ b) :=
  match a, b with
...
def d102864_udivmul_srem_to_zero_cexInputs : InstCombine.InputValuation
  ...
  d102864_udivmul_srem_to_zero_ivOfHVector
    (.cons (d102864_udivmul_srem_to_zero_valueOfNat 3 3) (.cons (d102864_udivmul_srem_to_zero_valueOfNat 3 6) .nil))

theorem d102864_udivmul_srem_to_zero_correct_counterexample_witness : ¬ (d102864_udivmul_srem_to_zero_src 3 ⊑ d102864_udivmul_srem_to_zero_tgt 3) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (d102864_udivmul_srem_to_zero_src 3) (d102864_udivmul_srem_to_zero_tgt 3) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (d102864_udivmul_srem_to_zero_src 3) (d102864_udivmul_srem_to_zero_tgt 3)).mp h
...
theorem d102864_udivmul_srem_to_zero_correct_counterexample :
    ∃ (w : Nat), ¬ (d102864_udivmul_srem_to_zero_src w ⊑ d102864_udivmul_srem_to_zero_tgt w) :=
  ⟨3, d102864_udivmul_srem_to_zero_correct_counterexample_witness⟩
