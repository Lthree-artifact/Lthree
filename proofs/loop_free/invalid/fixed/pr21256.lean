...
local instance pr21256_decRefSemVal (a b : LLVM.SemVal (BitVec 8)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance pr21256_decRefIntW (a b : LLVM.IntW 8) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 8)) (LLVM.SemVal (BitVec 8)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance pr21256_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 8)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 8)) (ImmediateUBOr (LLVM.IntW 8))
        inferInstance a b) := inferInstance
  exact d

local instance pr21256_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 8]) : Decidable (a ⊑ b) :=
  match a, b with
...
def pr21256_cexInputs : InstCombine.InputValuation
  ...
  pr21256_ivOfHVector
    (.cons (pr21256_valueOfNat 8 128) (.cons (pr21256_valueOfNat 8 255) .nil))

theorem pr21256_correct_counterexample_witness : ¬ (pr21256_src ⊑ pr21256_tgt) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (pr21256_src) (pr21256_tgt) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (pr21256_src) (pr21256_tgt)).mp h
...
theorem pr21256_correct_counterexample : ¬ (pr21256_src ⊑ pr21256_tgt) :=
  pr21256_correct_counterexample_witness
