...
local instance d158510_negate_nsw_shl_decRefSemVal (a b : LLVM.SemVal (BitVec 64)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance d158510_negate_nsw_shl_decRefIntW (a b : LLVM.IntW 64) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 64)) (LLVM.SemVal (BitVec 64)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance d158510_negate_nsw_shl_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 64)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 64)) (ImmediateUBOr (LLVM.IntW 64))
        inferInstance a b) := inferInstance
  exact d

local instance d158510_negate_nsw_shl_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 64]) : Decidable (a ⊑ b) :=
  match a, b with
...
def d158510_negate_nsw_shl_cexInputs : InstCombine.InputValuation
  ...
  d158510_negate_nsw_shl_ivOfHVector
    (.cons (d158510_negate_nsw_shl_valueOfNat 64 63) (.cons (d158510_negate_nsw_shl_valueOfNat 64 1) .nil))

theorem d158510_negate_nsw_shl_correct_counterexample_witness : ¬ (d158510_negate_nsw_shl_src ⊑ d158510_negate_nsw_shl_tgt) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (d158510_negate_nsw_shl_src) (d158510_negate_nsw_shl_tgt) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (d158510_negate_nsw_shl_src) (d158510_negate_nsw_shl_tgt)).mp h
...
theorem d158510_negate_nsw_shl_correct_counterexample : ¬ (d158510_negate_nsw_shl_src ⊑ d158510_negate_nsw_shl_tgt) :=
  d158510_negate_nsw_shl_correct_counterexample_witness
