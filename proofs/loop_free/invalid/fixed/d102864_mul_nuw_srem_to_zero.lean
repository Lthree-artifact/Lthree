...
def d102864_mul_nuw_srem_to_zero_src :=
  [llvm()| {
  llvm.func @d102864_mul_nuw_srem_to_zero_src(%x : i64, %y : i64) -> i64 {
  ^bb0(%x : i64, %y : i64):
    %mul = llvm.mul %x, %y overflow<nuw> : i64
    %mod = llvm.srem %mul, %y : i64
    llvm.return %mod : i64
  }
  }]

def d102864_mul_nuw_srem_to_zero_tgt :=
  [llvm()| {
  llvm.func @d102864_mul_nuw_srem_to_zero_tgt(%x : i64, %y : i64) -> i64 {
  ^bb0(%x : i64, %y : i64):
...
local instance d102864_mul_nuw_srem_to_zero_decRefSemVal (a b : LLVM.SemVal (BitVec 64)) :
    Decidable (a ⊑ b) :=
  match a, b with
...
local instance d102864_mul_nuw_srem_to_zero_decRefIntW (a b : LLVM.IntW 64) : Decidable (a ⊑ b) :=
  decidable_of_iff
    (@HRefinement.IsRefinedBy (LLVM.SemVal (BitVec 64)) (LLVM.SemVal (BitVec 64)) _ a b)
    (LLVM.IntW.isRefinedBy_iff a b).symm

local instance d102864_mul_nuw_srem_to_zero_decRefElt (a b : TyDenote.toType (InstCombine.LLVM.Ty.bitvec 64)) : Decidable (a ⊑ b) := by
  have d : Decidable
      (@HRefinement.IsRefinedBy (ImmediateUBOr (LLVM.IntW 64)) (ImmediateUBOr (LLVM.IntW 64))
        inferInstance a b) := inferInstance
  exact d

local instance d102864_mul_nuw_srem_to_zero_decRefRet (a b : HVector TyDenote.toType [InstCombine.LLVM.Ty.bitvec 64]) : Decidable (a ⊑ b) :=
  match a, b with
...
def d102864_mul_nuw_srem_to_zero_cexInputs : InstCombine.InputValuation
  ...
  d102864_mul_nuw_srem_to_zero_ivOfHVector
    (.cons (d102864_mul_nuw_srem_to_zero_valueOfNat 64 9223372036854775807) (.cons (d102864_mul_nuw_srem_to_zero_valueOfNat 64 2) .nil))

theorem d102864_mul_nuw_srem_to_zero_correct_counterexample_witness : ¬ (d102864_mul_nuw_srem_to_zero_src ⊑ d102864_mul_nuw_srem_to_zero_tgt) := by
  intro h
  have hyp : InstCombine.IsRefinedByOnIntWInputs (d102864_mul_nuw_srem_to_zero_src) (d102864_mul_nuw_srem_to_zero_tgt) :=
    (InstCombine.isRefinedByOnIntWInputs_iff (d102864_mul_nuw_srem_to_zero_src) (d102864_mul_nuw_srem_to_zero_tgt)).mp h
...
theorem d102864_mul_nuw_srem_to_zero_correct_counterexample : ¬ (d102864_mul_nuw_srem_to_zero_src ⊑ d102864_mul_nuw_srem_to_zero_tgt) :=
  d102864_mul_nuw_srem_to_zero_correct_counterexample_witness
